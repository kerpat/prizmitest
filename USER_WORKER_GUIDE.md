# УЛЬТИМАТИВНЫЙ ГАЙД ПО ИСПОЛЬЗОВАНИЮ ПРИЛОЖЕНИЯ GoGoBike
...
4. **Перейдите по ссылке**: Бот отправит ссылку на веб-приложение (URL настраивается через переменную окружения `APP_BASE_URL`).
5. **Заполните профиль**:
   - Имя и фамилия
   - Адрес проживания
...
1. **URL**: Откройте admin.html (URL настраивается через переменную окружения `APP_BASE_URL`).
